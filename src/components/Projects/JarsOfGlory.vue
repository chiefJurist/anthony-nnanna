<template>
    <a
        href="https://jarsofglory.org"
        target="blank"
        class="block group relative bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-3"
    >
        <div>
            <img src="/public/img/jog.png" alt="jog" />
        </div>

        <div class="p-8 lg:p-10">
            <h3 class="text-2xl font-bold text-gray-900 dark:text-white">
                Jars Of Glory NGO Website
            </h3>

            <p class="mt-2 text-cyan-600 dark:text-cyan-400 font-semibold">
                Nonprofit Digital Platform · Lead Developer
            </p>

            <div class="mt-6 flex flex-wrap gap-2">
                <span
                    class="px-3 py-1 bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 rounded-lg text-sm font-medium"
                    >WordPress (Headless)</span
                >
                <span
                    class="px-3 py-1 bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 rounded-lg text-sm font-medium"
                    >Vue.js</span
                >
                <span
                    class="px-3 py-1 bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 rounded-lg text-sm font-medium"
                    >TypeScript</span
                >
            </div>

            <ul class="mt-6 space-y-3 text-gray-700 dark:text-gray-300">
                <li class="flex items-start">
                    <span class="text-cyan-600 dark:text-cyan-400 mr-2">▹</span>
                    Designed and built the organisation's digital presence using a headless
                    WordPress architecture with a Vue.js/TypeScript front end.
                </li>
                <li class="flex items-start">
                    <span class="text-cyan-600 dark:text-cyan-400 mr-2">▹</span>
                    Built a custom donation and payment integration enabling supporters to
                    contribute directly through the site.
                </li>
                <li class="flex items-start">
                    <span class="text-cyan-600 dark:text-cyan-400 mr-2">▹</span>
                    <strong class="text-cyan-600 dark:text-cyan-400"
                        >Developed an event registration and booking system reducing manual
                        coordination for the events team</strong
                    >
                </li>
            </ul>
        </div>
    </a>
</template>
